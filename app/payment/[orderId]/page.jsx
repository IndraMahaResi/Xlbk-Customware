...
export default function PaymentPage() {
  ...
  // 🟢 STATE BARU: Untuk mengontrol menu ubah metode pembayaran
  ...
  const WALLET_ADDRESSES = {
    BTC:
      storeSettings?.cryptoBtc || "bc1qar9fgrkghr6v58qelc3cdjkptyw8j3gh95w24s",
    USDT:
      storeSettings?.cryptoUsdt || "0xb1bFa84d196aB9F32D07F770F3c5712501d5903c",
  };
        ...
        try {
          ...
          if (res.ok) {
            ...
            if (currentOrder.status !== "NEED_VERIFICATION") {
              ...
              toast.success(
                "Ongkos kirim telah diupdate! Silakan lanjutkan pembayaran.",
                { duration: 5000 },
              );
  ...
  // 🔥 FUNGSI UBAH METODE PEMBAYARAN DIPERBAIKI
  const handleChangeMethod = async (newMethod) => {
    ...
    try {
      ...
      // Request ke endpoint PATCH yang kita gunakan untuk upload bukti
      const res = await fetch(`/api/orders/${params.orderId}`, {
        method: "PATCH",
      ...
      if (res.ok) {
        toast.success("Metode pembayaran berhasil diubah!", { id: toastId });
        setShowChangeMethod(false);

        // 🟢 PAKSA RE-RENDER DENGAN MENGUPDATE STATE LOKAL
        if (order) setOrder({ ...order, paymentMethod: newMethod });
        if (payment) setPayment({ ...payment, method: newMethod });

        // Tetap panggil fetchOrderData untuk memastikan sync penuh dgn database
        fetchOrderData();
      } else {
        toast.error("Gagal mengubah metode pembayaran", { id: toastId });
      }
    } catch (error) {
      toast.error("Terjadi kesalahan sistem saat mengubah metode");
    }
  };

  // 🔥 FUNGSI UPLOAD BUKTI BAYAR (LOGIKA STATUS DIPERBAIKI)
  const handlePaymentProof = async (e) => {
    ...
    setUploading(true);
    try {
      // 🟢 LOGIKA: Jika FULL PAYMENT atau PELUNASAN -> PAID. Jika DP -> VERIFIED.
      const activePaymentType = order?.paymentType || urlPayType || "FULL";
      const newPaymentStatus =
        isPelunasan || activePaymentType === "FULL" ? "PAID" : "VERIFIED";
      ...
      if (updateRes.ok) {
        toast.success(
          newPaymentStatus === "PAID"
            ? "Pembayaran Lunas berhasil diterima! 🎉"
            : "Bukti DP berhasil diterima! 🎉",
        );
        fetchOrderData();

        const paymentTypeStr = isPelunasan
          ? "Pelunasan Tagihan"
          : activePaymentType === "DP"
            ? "Uang Muka (DP 50%)"
            : "Pembayaran Penuh";
  ...
  const calculateLiveCrypto = (amountIdr, method) => {
    ...
    if (method === "BTC" && cryptoRates.BTC > 0)
      return (amountIdr / cryptoRates.BTC).toFixed(8);
    if (method === "USDT" && cryptoRates.USDT > 0)
      return (amountIdr / cryptoRates.USDT).toFixed(2);
    return "0";
  };
  ...
  if (loading) {
    return (
      <div className="pt-20 min-h-screen flex flex-col items-center justify-center bg-slate-950">
        ...
        <p className="text-slate-400 animate-pulse tracking-widest text-sm uppercase font-bold">
  ...
  // 🟢 PRIORITASKAN MEMBACA METHOD DARI ORDER
  const rawMethod = order?.paymentMethod || payment?.method || "BANK_TRANSFER";
  ...
  // 🟢 LOGIKA HALAMAN SUKSES DIPERBAIKI:
  // Jika sedang mode Pelunasan, JANGAN tampilkan layar sukses kecuali statusnya sudah PAID.
  const isPaid = isPelunasan
    ? currentPayStatus === "PAID"
    : currentPayStatus === "PAID" || currentPayStatus === "VERIFIED";
          ...
          {isVerificationNeeded ? (
            <div className="animate-in fade-in zoom-in-95 duration-500 max-w-3xl mx-auto mt-10">
              <div className="bg-slate-900/80 border border-slate-700/80 backdrop-blur-xl rounded-3xl p-8 md:p-14 text-center shadow-2xl relative overflow-hidden">
                ...
                <p className="text-slate-400 font-mono mb-4 text-sm">
                  Invoice:{" "}
                ...
                <p className="text-slate-300 leading-relaxed mb-6 max-w-lg mx-auto bg-slate-800/50 p-6 rounded-2xl border border-slate-700">
                  Pesanan internasional Anda sedang ditinjau. Kami sedang
                  mengkalkulasi total biaya pengiriman ke negara Anda. Mohon
                  tunggu informasi dan instruksi lebih lanjut dari kami.
                </p>

                <div className="flex items-center justify-center gap-2 text-amber-400 mb-8 font-medium text-sm animate-pulse">
                  <ArrowPathIcon className="w-5 h-5 animate-spin" />
                  Halaman ini akan otomatis terbuka jika ongkir sudah
                  diupdate...
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                  <button
                    onClick={() =>
                      window.open(
                        `https://wa.me/${adminPhone}?text=Halo Admin, saya ingin menanyakan status perhitungan ongkir untuk pesanan internasional saya dengan Invoice: *${order?.invoiceNumber}*. Terima kasih!`,
                        "_blank",
                      )
                    }
                ...
                {isPelunasan && (
                  <div className="mt-4">
                    <span className="inline-flex items-center gap-1 px-4 py-1.5 bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xs font-bold uppercase tracking-wider rounded-full shadow-lg shadow-amber-500/10">
                      <CheckCircleIcon className="w-4 h-4" /> Mode Pembayaran
                      Pelunasan
                    </span>
                  </div>
                )}
                ...
                <div className="lg:col-span-7 space-y-6">
                  {!isPelunasan && countdown !== 0 && (
                    <div className="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-6 text-center shadow-lg relative overflow-hidden">
                      ...
                      <p className="text-amber-400/80 font-medium mb-2 text-sm uppercase tracking-wider relative z-10">
                        Batas Waktu Pembayaran
                      </p>
                      <div className="flex justify-center items-center gap-3 text-3xl font-extrabold text-amber-400 font-mono relative z-10">
                        <div className="bg-amber-950/50 px-3 py-2 rounded-lg border border-amber-500/30">
                  ...
                  <div className="bg-slate-900/80 border border-slate-700/80 backdrop-blur-xl rounded-3xl p-8 shadow-2xl relative overflow-hidden">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4 border-b border-slate-700/50 pb-4">
                      <h2 className="text-xl font-bold text-slate-100 flex items-center gap-3">
                        ...
                        Instruksi ({payMethod.replace("_", " ")})
                      </h2>
                    </div>

                    {/* 🟢 TOMBOL GANTI METODE YANG LEBIH BESAR & JELAS */}
                    {!isPelunasan && (
                      <div className="mb-8">
                        <button
                          onClick={() => setShowChangeMethod(!showChangeMethod)}
                          className={`w-full py-4 rounded-xl border-2 font-bold text-base transition-all shadow-md flex items-center justify-center gap-2 ${
                            showChangeMethod
                        ...
                        >
                          <ArrowPathIcon
                            className={`w-5 h-5 ${showChangeMethod ? "" : "animate-spin-slow"}`}
                          />
                          {showChangeMethod
                            ? "Batal Mengganti Metode"
                            : "🔄 Klik Disini Untuk Ganti Metode Pembayaran"}
                        </button>
                      </div>
                    )}

                    {/* 🟢 PANEL PEMILIHAN METODE PEMBAYARAN BARU */}
                    {showChangeMethod && (
                      <div className="mb-8 p-6 bg-slate-950 rounded-2xl border-2 border-blue-500/30 animate-in fade-in slide-in-from-top-4 duration-300 shadow-[0_0_30px_rgba(37,99,235,0.1)]">
                        <p className="text-sm font-bold text-slate-200 mb-4 uppercase tracking-wider text-center">
                          Pilih Metode Baru Anda:
                        </p>
                        <div className="grid grid-cols-2 gap-4">
                          {[
                            { method: "QRIS", label: "QRIS Scan", icon: "📱" },
                            {
                            ...
                            { method: "BTC", label: "Bitcoin", icon: "₿" },
                          ].map((p) => (
                            <button
                              key={p.method}
                              onClick={() => handleChangeMethod(p.method)}
                              className={`py-4 px-3 flex flex-col items-center justify-center gap-2 text-sm font-bold rounded-xl border-2 transition-all ${
                                payMethod === p.method
                                  ? "bg-blue-600/20 border-blue-500 text-blue-400 shadow-[0_0_15px_rgba(37,99,235,0.2)]"
                                  : "bg-slate-800 border-slate-700 text-slate-300 hover:border-slate-500 hover:text-white hover:bg-slate-700"
                              }`}
                            >
                    ...
                    {payMethod === "QRIS" ? (
                      ...
                      <div className="animate-in fade-in duration-500 space-y-6 text-center flex flex-col items-center">
                        ...
                        <div className="w-full bg-slate-800 border border-slate-700 rounded-2xl p-6 text-center relative group hover:border-amber-500/50 transition-colors">
                          ...
                          <p className="text-xs text-slate-400 uppercase font-bold mb-3 tracking-wider mt-4">
                            Alamat Wallet Bitcoin
                          </p>
                          <p className="font-mono text-amber-400 break-all mb-5 text-sm sm:text-base bg-slate-950 p-4 rounded-lg border border-slate-700">
                          ...
                          <button
                            ...
                            className="inline-flex items-center gap-2 bg-amber-500/10 text-amber-400 px-5 py-2.5 rounded-xl text-sm font-bold border border-amber-500/30 hover:bg-amber-500 hover:text-white transition-all shadow-md group-hover:shadow-amber-500/20"
                          >
                        ...
                        <div className="w-full bg-slate-800/30 p-5 rounded-2xl border border-slate-700/50 text-center">
                          ...
                          <span className="text-amber-400 font-mono font-black text-2xl sm:text-3xl tracking-tight">
                            {isCalculating ? (
                              ...
                              `${formattedCryptoDisplay} BTC`
                            )}
                          </span>
                          <p className="text-xs text-amber-500 font-bold mt-2">
                            PENTING: Pastikan Anda menggunakan Jaringan BTC
                            (Bitcoin Network).
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="animate-in fade-in duration-500 space-y-6 text-center flex flex-col items-center">
                        ...
                        <div className="w-full bg-slate-800 border border-slate-700 rounded-2xl p-6 text-center relative group hover:border-emerald-500/50 transition-colors">
                          ...
                          <p className="text-xs text-slate-400 uppercase font-bold mb-3 tracking-wider mt-4">
                            Alamat Wallet Tether (USDT)
                          </p>
                          <p className="font-mono text-emerald-400 break-all mb-5 text-sm sm:text-base bg-slate-950 p-4 rounded-lg border border-slate-700">
                          ...
                          <button
                            ...
                            className="inline-flex items-center gap-2 bg-emerald-500/10 text-emerald-400 px-5 py-2.5 rounded-xl text-sm font-bold border border-emerald-500/30 hover:bg-emerald-500 hover:text-white transition-all shadow-md group-hover:shadow-emerald-500/20"
                          >
                            <DocumentDuplicateIcon className="w-5 h-5" />
                            Salin Alamat Wallet
                          </button>
                        ...
                        <div className="w-full bg-slate-800/30 p-5 rounded-2xl border border-slate-700/50 text-center">
                          ...
                          <span className="text-emerald-400 font-mono font-black text-3xl tracking-tight">
                            {isCalculating ? (
                              ...
                              `${formattedCryptoDisplay} USDT`
                            )}
                          </span>
                          <p className="text-xs text-emerald-500 font-bold mt-2">
                            PENTING: Gunakan jaringan BNB Smart Chain (BEP 20).
                          </p>
                        </div>
                      </div>
                    ) : (
                      <div className="space-y-4 animate-in fade-in duration-500">
                        {BANKS.length > 0 ? (
                          BANKS.map((bankObj, idx) => (
                            <div
                              key={idx}
                              className="bg-slate-800 border border-slate-700 rounded-xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 group hover:border-blue-500/50 transition-colors"
                            >
                              <div>
                              ...
                              <div className="flex items-center gap-3">
                                <p className="font-mono text-xl font-bold text-blue-400 tracking-wider">
                                  {bankObj.number}
                                </p>
                                <button
                                  onClick={() =>
                                    copyToClipboard(
                                      bankObj.number,
                                      `No. Rekening ${bankObj.bank}`,
                                    )
                                  }
                                  className="p-2.5 bg-slate-900 rounded-lg border border-slate-700 hover:border-blue-500 transition-all"
                                >
                          ...
                          <div className="text-slate-400 text-center py-6 border border-slate-700 border-dashed rounded-xl bg-slate-800/30 text-sm italic">
                            Belum ada bank yang dikonfigurasi. Silakan hubungi
                            Admin.
                          </div>
                        )}
                      </div>
                  ...
                  <div className="bg-gradient-to-br from-blue-900/40 to-slate-900/80 border border-blue-500/30 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
                    ...
                    {order?.countryOrigin !== "Indonesia" && (
                      <div className="text-xs text-amber-400 mb-4 bg-amber-500/10 p-2 rounded-lg border border-amber-500/20 inline-block">
                        *Total Asli termasuk ongkos kirim ke{" "}
                        {order?.countryOrigin || "Luar Negeri"}
                      </div>
                    )}
                    ...
                    <div className="border-t border-slate-700/50 pt-6 space-y-3 relative z-10">
                      {order?.items?.map((item, idx) => {
                        ...
                        return (
                          <div
                            key={idx}
                            className="flex justify-between text-sm text-slate-400 gap-4"
                          >
                            <span className="truncate">
                              {itemQty}x{" "}
                              {item.product?.name || "Produk Customwear"}
                            </span>
                  ...
                  <div className="bg-slate-900/80 border border-slate-700/80 backdrop-blur-xl rounded-3xl p-8 shadow-2xl">
                    ...
                    <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-4 mb-5 flex items-start gap-3">
                      ...
                      <p className="text-blue-400 text-xs">
                        <strong>PENTING:</strong> Pastikan Anda telah mengunduh{" "}
                        <strong>Invoice</strong> (tombol di atas) sebagai
                        referensi dan bukti tagihan sebelum Anda mengkonfirmasi
                    ...
                    <div className="relative">
                      ...
                      />
                      <div
                        className={`border-2 border-dashed rounded-2xl p-10 text-center transition-all duration-300 flex flex-col items-center justify-center min-h-[160px]
                      ...
                      >
                        {uploading ? (
                          <div className="flex flex-col items-center">
                            <ArrowPathIcon className="w-10 h-10 text-blue-400 animate-spin mb-4" />
                          ...
                          <div className="flex flex-col items-center">
                            ...
                            <p className="text-sm font-bold text-slate-200 mb-1">
                              Klik di Sini Jika Sudah Transfer
                            </p>
                            <p className="text-xs text-slate-500">
                              Pilih file / screenshot bukti transaksi
                            </p>
                          </div>
                        )}
                      </div>
              ...
              <div className="bg-slate-900/80 border border-slate-700/80 backdrop-blur-xl rounded-3xl p-10 md:p-14 text-center shadow-2xl relative overflow-hidden">
                ...
                <div className="flex flex-col gap-4 relative z-10 max-w-md mx-auto">
                  ...
                  <div className="flex flex-col sm:flex-row gap-4">
                    <button
                      onClick={generateInvoice}
                      className="flex-1 bg-slate-800 hover:bg-slate-700 text-white py-4 rounded-2xl font-bold border border-slate-600 transition-all flex justify-center items-center gap-2.5 shadow-md"
                    >
                      <DocumentArrowDownIcon className="w-5 h-5" /> Unduh
                      Invoice PDF
                    </button>
                    <button
                      onClick={() => router.push("/")}
                      className="flex-1 flex justify-center items-center bg-blue-600 text-white py-4 rounded-2xl font-bold hover:bg-blue-500 transition-all shadow-glow text-center"
                    >
                      Kembali ke Beranda
                    </button>
                  </div>
