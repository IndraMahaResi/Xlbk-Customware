...
import {
  ...
  Cog6ToothIcon,
  PhotoIcon,
  MegaphoneIcon // 🔥 Ikon untuk Event & Promo
...
export default function DashboardLayout({ children }) {
  ...
  const navigation = [
    ...
    { name: 'Verifikasi LN', href: '/dashboard/international', icon: GlobeAltIcon }, 
    { name: 'Katalog Produk', href: '/dashboard/products', icon: CubeIcon },
    { name: 'Event & Promo', href: '/dashboard/promo', icon: MegaphoneIcon }, // 🟢 Menu Baru
    { name: 'Hasil Karya', href: '/dashboard/hasil-karya', icon: PhotoIcon }, // 🟢 Menu Baru
    { name: 'Testimoni', href: '/dashboard/testimonials', icon: ChatBubbleLeftRightIcon },
    { name: 'Pengaturan Toko', href: '/dashboard/settings', icon: Cog6ToothIcon },
  ...
  if (!mounted || !user) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center">
        ...
        <p className="mt-4 text-slate-500 text-xs font-bold tracking-widest uppercase">Memverifikasi Sesi</p>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200">
      <aside className="fixed inset-y-0 left-0 w-64 bg-slate-900/50 backdrop-blur-xl border-r border-slate-800 hidden md:flex flex-col z-30">
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
          {navigation.map((item) => {
            ...
            return (
              <Link key={item.name} href={item.href} className={`flex items-center px-4 py-3 rounded-xl transition-all duration-300 group ${isActive ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-400 hover:bg-slate-800/50 hover:text-slate-100'}`}>
        ...
        <div className="p-4 border-t border-slate-800/50">
          <button onClick={handleLogout} className="w-full flex items-center justify-center px-4 py-3 text-sm font-bold text-rose-400 bg-rose-500/10 border border-rose-500/20 rounded-xl hover:bg-rose-500 hover:text-white transition-all duration-300">
            <ArrowRightOnRectangleIcon className="h-5 w-5 mr-2" /> Keluar Sesi
          </button>
        </div>
      </aside>
      ...
      <div className="md:ml-64 flex flex-col min-h-screen relative z-10">
        <header className="h-20 bg-slate-950/80 backdrop-blur-md border-b border-slate-800 sticky top-0 z-20 flex items-center justify-between px-8">
          ...
          <div className="flex items-center gap-4 bg-slate-900/50 py-1.5 px-3 rounded-full border border-slate-800">
